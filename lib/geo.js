...
(function (exports) {
  ...
  /**
   * Get user location via browser geolocation API with sessionStorage caching.
   * Returns a Promise that resolves with { lat, lng } or rejects on failure.
   */
  function getUserLocation() {
    var CACHE_KEY = "lnv_user_location";
    ...
    return new Promise(function (resolve, reject) {
      navigator.geolocation.getCurrentPosition(
        function (pos) {
          ...
          if (typeof sessionStorage !== "undefined") {
            try { sessionStorage.setItem(CACHE_KEY, JSON.stringify(loc)); } catch (e) { /* quota */ }
          }
          resolve(loc);
        },
        function (err) {
  ...
  /**
   * Compute distance in miles from a point to a venue (using venue coords or neighborhood fallback).
   */
  function distanceToVenueMiles(lat, lng, venue, citySlug) {
    var coords = getCoordsForVenue(venue, citySlug);
    if (!coords || !haversineMiles) return null;
    return haversineMiles(lat, lng, coords[0], coords[1]);
  }
  ...
  exports.distanceToVenueMiles = distanceToVenueMiles;
