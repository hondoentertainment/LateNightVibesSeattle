...
(function () {
  ...
  function updateURL(name) {
    ...
    if (name) url.searchParams.set("venue", name);
  ...
  function open(venue) {
    ...
    var cityName = (window.LNVCities && window.LNVCities.getCurrentCity().name) || "Seattle";
    const googleSearch = "https://www.google.com/maps/search/?api=1&query=" + encodeURIComponent(venueName + " " + area + " " + cityName);
    const yelpSearch = "https://www.yelp.com/search?find_desc=" + encodeURIComponent(venueName) + "&find_loc=" + encodeURIComponent(area + ", " + cityName);
  ...
  function openFromURL(venues) {
    const param = new URLSearchParams(window.location.search).get("venue");
    if (!param) return;
    const query = param.trim().toLowerCase();
    const match = venues.find(function (v) {
      return normalizeValue(v.Name).toLowerCase() === query;
    });
    if (match) open(match);
  }
