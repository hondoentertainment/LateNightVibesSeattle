...
(function (exports) {
  ...
  function saveEmergencyContacts(contacts) {
    ...
    try { localStorage.setItem(CONTACTS_KEY, JSON.stringify(trimmed)); } catch (e) { /* quota */ }
    return trimmed;
  }
  ...
  function enableBuddyMode(buddyName) {
    ...
    try { localStorage.setItem(BUDDY_KEY, JSON.stringify(data)); } catch (e) { /* quota */ }
  ...
  function disableBuddyMode() {
    try { localStorage.removeItem(BUDDY_KEY); } catch (e) { /* noop */ }
  }
  ...
  function checkBuddyStatus() {
    ...
    } catch (e) { return { active: false, buddyName: "" }; }
  }

  function checkIn() {
    var ts = Date.now();
    try { localStorage.setItem(CHECKIN_KEY, JSON.stringify(ts)); } catch (e) { /* quota */ }
    return ts;
  }
