/**
 * Late Night Vibes — Core pure logic functions.
 *
...
(function (exports) {
  ...
  function loadDataFromCSV(text) {
    ...
    const hasLat = headers.indexOf("Latitude") !== -1;
    const hasLng = headers.indexOf("Longitude") !== -1;
    ...
    return rows.slice(1)
      ...
      .map((row) => {
        ...
        if (hasLat && r["Latitude"]) {
          r.lat = parseFloat(r["Latitude"]);
        }
        if (hasLng && r["Longitude"]) {
          r.lng = parseFloat(r["Longitude"]);
        }
        return r;
      });
  }
