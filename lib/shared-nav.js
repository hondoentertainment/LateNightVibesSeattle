...
(function () {
  ...
  const BOTTOM_NAV_ITEMS = [
    ...
    {
      href: "mood.html", label: "Mood", page: "mood", ariaLabel: "Mood match",
  ...
  function buildDesktopNav(currentPage, options) {
    ...
    // Render primary nav links
    PRIMARY_NAV_LINKS.forEach(function (link) {
    ...
    // Build "More" dropdown for overflow links
    var moreLinks = MORE_NAV_LINKS.filter(function (link) {
      return link.page !== currentPage;
    });

    // Also check if admin link should be added
    var adminLink = NAV_LINKS.find(function (l) { return l.adminOnly; });
    ...
    if (moreLinks.length > 0) {
      ...
      toggle.className = "nav-pill nav-more-toggle";
      toggle.setAttribute("aria-expanded", "false");
      toggle.setAttribute("aria-haspopup", "true");
      toggle.innerHTML = 'More <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" aria-hidden="true"><path d="M6 9l6 6 6-6"/></svg>';
      ...
      if (adminLink) {
        ...
        adminA.href = cityLink(adminLink.href);
        adminA.className = "nav-more-item nav-admin";
        adminA.textContent = adminLink.label;
        if (options && options.adminId) adminA.id = options.adminId;
        dropdown.appendChild(adminA);
      }
