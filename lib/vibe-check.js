/**
 * Late Night Vibes — Vibe Check (crowd-sourced energy meter).
 *
...
(function (exports) {
  ...
  function _writeAll(checks) {
    try {
      ...
      if (sm && sm.safePut) {
        sm.safePut(_scopedKey(), JSON.stringify(checks));
      } else {
        localStorage.setItem(_scopedKey(), JSON.stringify(checks));
      }
