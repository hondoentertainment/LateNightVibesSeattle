/**
 * Shared high-impact feature logic for Late Night Vibes.
...
(function (exports) {
  ...
  var geo = (typeof require !== "undefined") ? require("./geo") : (typeof window !== "undefined" && window.LNVGeo ? window.LNVGeo : null);
  ...
  /**
   * Estimate travel minutes between two venues using Haversine distance when
   * coordinates are available, falling back to the naive same-area / cross-area
   * heuristic when they are not.
   *
   * @param {object} venueA - first venue object
   * @param {object} venueB - second venue object
   * @param {string} [citySlug] - city slug for coordinate lookup (default "seattle")
   * @returns {number} estimated travel time in minutes
   */
  function estimateTravelMinutes(venueA, venueB, citySlug) {
    var slug = citySlug || "seattle";
    if (geo && geo.venueToVenueDistanceMiles) {
      var miles = geo.venueToVenueDistanceMiles(venueA, venueB, slug);
      if (miles != null) {
        // Walkable distance: ~20 min per mile on foot
        if (miles <= WALKABLE_MILES) {
          return Math.max(2, Math.ceil(miles * 20));
        }
        // Driving: base 5 min + 4 min per mile
        return Math.ceil(5 + miles * 4);
      }
    }
    // Fallback: naive area-based estimate
