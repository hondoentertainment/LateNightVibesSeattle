...
(function (exports) {
  ...
  var geo = (typeof require !== "undefined") ? require("./geo") : (typeof window !== "undefined" && window.LNVGeo ? window.LNVGeo : {});
  ...
  function estimateTravelMinutes(venueA, venueB, citySlug) {
    var getCoordsForVenue = geo.getCoordsForVenue;
    if (getCoordsForVenue && haversineMiles) {
      var slug = citySlug || "seattle";
      var coordsA = getCoordsForVenue(venueA, slug);
      var coordsB = getCoordsForVenue(venueB, slug);
      if (coordsA && coordsB) {
        var miles = haversineMiles(coordsA[0], coordsA[1], coordsB[0], coordsB[1]);
        if (miles != null) {
          // Walking: ~20 min/mile; Driving: ~2 min/mile + 3 min base
          if (miles <= WALKABLE_MILES) {
            return Math.max(2, Math.ceil(miles * 20));
          }
          return Math.max(3, Math.ceil(miles * 2 + 3));
        }
      }
    }
    // Fallback: simplified area-based heuristic
