...
(function (exports) {
  ...
  function _evaluateProgress(challenge) {
    ...
    if (criteria.category) {
      ...
      var catProgress = Math.min(weekVisits.length, challenge.target);
      return Math.max(challenge.progress, catProgress);
    }

    // Discovery: new venues
    if (criteria.newVenues) {
      var newProgress = Math.min(weekVisits.length, challenge.target);
      return Math.max(challenge.progress, newProgress);
    }
    ...
    if (criteria.consecutiveNights) {
      ...
      if (curConsec > maxConsec) maxConsec = curConsec;
      var streakProgress = Math.min(nights.length > 0 ? maxConsec : 0, challenge.target);
      return Math.max(challenge.progress, streakProgress);
    }
    ...
    if (criteria.weekendNights) {
      ...
      var weekendProgress = Math.min(Object.keys(weekendDays).length, challenge.target);
      return Math.max(challenge.progress, weekendProgress);
    }
