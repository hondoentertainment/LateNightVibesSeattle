/**
 * Crawl History — "Been There" tracker for Late Night Vibes.
 *
...
(function (exports) {
  ...
  function _storageManager() {
    if (typeof module !== "undefined") return require("./storage-manager");
    return (typeof window !== "undefined" && window.LNVStorageManager) || null;
  }
  ...
  function saveHistory(history) {
    try {
      if (typeof localStorage !== "undefined") {
        var sm = _storageManager();
        if (sm && sm.safePut) {
          sm.safePut(_scopedKey(), JSON.stringify(history));
        } else {
          localStorage.setItem(_scopedKey(), JSON.stringify(history));
        }
      }
    } catch (_) {}
  }
